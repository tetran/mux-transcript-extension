...
### パターン A：DeepL 翻訳を使う（推奨）

1. ユーザーは拡張機能の設定画面を開き、DeepL Free API キーを入力して保存する
2. ユーザーは Laravel Learn のレッスンページを開く（ブラウザ翻訳は不要）
3. 拡張が自動で動画直下に字幕欄を追加し、Transcript テキストを DeepL で日本語訳して表示する
4. 再生に合わせて字幕が自動更新される
5. Transcript 行のクリックでシークできる既存ページ機能はそのまま（拡張は干渉しない）

### パターン B：Chrome のページ翻訳を使う（API キー不要）

1. ユーザーは Laravel Learn のレッスンページを開く
2. Chrome の「このページを翻訳」機能で日本語に翻訳する
3. 拡張が自動で動画直下に字幕欄を追加し、翻訳済みの Transcript 表示テキストをそのまま字幕として使う
4. 再生に合わせて字幕が自動更新される
5. Transcript 行のクリックでシークできる既存ページ機能はそのまま（拡張は干渉しない）
